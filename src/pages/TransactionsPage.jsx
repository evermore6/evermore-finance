...
export default function TransactionsPage() {
  ...
  const handleEdit = async (data) => {
    setAddLoading(true)
    const { error } = await updateTransaction(editItem.id, data)
    setAddLoading(false)
  ...
  const handleExport = async (format) => {
    ...
    setShowExport(false)
    if (format === 'csv')   { exportToCSV(displayed, `evermore-${viewDate.year}-${viewDate.month + 1}`); toast.success('Exported CSV!') }
    if (format === 'excel') { await exportToExcel(displayed, `evermore-${viewDate.year}-${viewDate.month + 1}`); toast.success('Exported Excel!') }
    if (format === 'pdf')   { await exportToPDF(displayed, monthLabel, `evermore-report-${viewDate.year}-${viewDate.month + 1}`); toast.success('Exported PDF!') }
  }
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Transactions"
        subtitle={`${displayed.length} transaction${displayed.length !== 1 ? 's' : ''} this month`}
        action={
          <div className="flex items-center gap-2">
            <div className="relative">
              <Button variant="secondary" size="sm" icon={Download} onClick={() => setShowExport(v => !v)}>
                Export
              </Button>
              {showExport && (
                <motion.div
                  ...
                  className="absolute right-0 top-10 z-30 glass-card rounded-xl p-2 w-40 shadow-glass-md"
                >
                  {['csv', 'excel', 'pdf'].map(f => (
                    <button
                      key={f}
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => navigateMonth(1)} disabled={viewDate.year === now.getFullYear() && viewDate.month === now.getMonth()}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="relative flex-1 min-w-48">
          ...
          />
          {search && (
            <button onClick={() => setSearch('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-warmgray-400 hover:text-warmgray-600">
              <X size={13} />
            </button>
          )}
        </div>
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Add Transaction" size="md">
        <TransactionForm onSubmit={handleAdd} loading={addLoading} />
      </Modal>
