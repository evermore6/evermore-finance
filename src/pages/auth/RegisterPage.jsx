  ...
  return (
    <div className="min-h-screen gradient-bg flex items-center justify-center p-4">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-32 -left-32 w-96 h-96 rounded-full bg-sage-200/30 dark:bg-sage-900/20 blur-3xl" />
        <div className="absolute -bottom-32 -right-32 w-96 h-96 rounded-full bg-peach-200/30 dark:bg-peach-900/20 blur-3xl" />
      </div>
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-[var(--text-muted)] mt-1">Start your financial journey</p>
        </div>

        <div className="glass-card rounded-3xl p-7 shadow-glass-md">
          <h2 className="font-display text-xl font-semibold text-[var(--text-primary)] mb-5">Create account</h2>

          <Button variant="secondary" className="w-full mb-4 justify-center" onClick={handleGoogle} loading={googleLoading}>
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            <div>
              <label className="text-sm font-medium text-warmgray-600 dark:text-warmgray-300 block mb-1.5">Full Name</label>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPw(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-warmgray-400 hover:text-warmgray-600 transition-colors">
            ...
            <div>
              <label className="text-sm font-medium text-warmgray-600 dark:text-warmgray-300 block mb-1.5">Confirm Password</label>
              <div className="relative">
                <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-warmgray-400" />
                <input type={showPw ? 'text' : 'password'} value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="Repeat password" required
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-warmgray-200 dark:border-warmgray-700 bg-white/60 dark:bg-warmgray-900/40 text-sm text-[var(--text-primary)] focus:outline-none focus:border-sage-400 focus:ring-1 focus:ring-sage-300 transition-all" />
              </div>
            </div>

            <Button type="submit" variant="primary" className="w-full mt-1" size="lg" loading={loading}>
              Create Account
            </Button>
          </form>
          ...
          <p className="text-center text-sm text-[var(--text-muted)] mt-5">
            ...
            <Link to="/login" className="text-sage-600 dark:text-sage-300 font-medium hover:underline">Sign in</Link>
          </p>
        </div>
      </motion.div>
    </div>
