...
function DebtCard({ debt, onEdit, onDelete, onPay }) {
  ...
  const progress  = Math.min(((debt.paid_amount || 0) / debt.amount) * 100, 100)
  ...
  return (
    ...
    >
      ...
      {/* Progress bar kalau ada pembayaran */}
      ...
      {debt.notes && (
        <p className="text-xs text-[var(--text-muted)] mb-2 line-clamp-2 italic">"{debt.notes}"</p>
      )}
      ...
      <div className="flex items-center justify-between pt-2 border-t border-[var(--border)] mt-1">
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity ml-auto">
          <button
            onClick={() => onEdit(debt)}
            className="flex items-center gap-1 text-xs text-warmgray-500 hover:text-sage-600 px-2 py-1 rounded-lg hover:bg-sage-50 dark:hover:bg-sage-900/20 transition-colors"
          >
            <Edit2 size={12} /> Edit
          </button>
          <button
            onClick={() => onDelete(debt.id)}
            className="flex items-center gap-1 text-xs text-warmgray-500 hover:text-red-500 px-2 py-1 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
          >
...
export default function DebtsPage() {
  ...
  // Summary — hanya hitung yang belum lunas
  ...
  const handlePayment = async (payData) => {
    ...
    try {
      ...
      // 1. Update paid_amount di debt
      const { error: debtErr } = await payDebt({
        debt_id:    debt.id,
      ...
      // 2. Adjust saldo wallet
      if (payData.wallet_id) {
        // Payable (hutang): keluar → negatif
        // Receivable (piutang): masuk → positif
        const delta = isPayable
          ? -(actualAmount + adminFee)
          :  actualAmount
      ...
      // 4. Catat admin fee sebagai expense terpisah kalau ada
      ...
      toast.success(
        payData.pay_mode === 'full'
          ...
          : `💰 ${formatCurrency(actualAmount)} berhasil dicatat`
      )
      setPayItem(null)
    } catch (e) {
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState
            ...
            description={
              tab === 'payable'
                ? 'Tambahkan hutang yang perlu kamu bayar.'
                : 'Tambahkan piutang yang perlu ditagih.'
            }
            action={
